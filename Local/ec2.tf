resource "aws_instance" "vm-1" {
  #ami                    = data.aws_ami.joindevops-ami.id #th queried data is using here
  ami = local.ami_id
  #instance_type          = var.instance_type # this can be overwritten
  instance_type = local.ami_id # this can't be overwritten
  tags                   = var.tags
